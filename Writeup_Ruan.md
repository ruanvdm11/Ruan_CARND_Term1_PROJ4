...
* 5) Each array for the polygon lines are stored in order to obtain an average so that small inconsistencies in the images do not play such a big role. The code can be seen below:

```python

leftData = np.zeros((1,720))
def leftLineAvg(leftLine):
    ...
    if len(leftData)>=15:
        summ = np.zeros((1,720))
        for j in range(len(leftData)-15,len(leftData)):
            summ += leftData[j]
        leftavg = summ/(15)   
...
[![alt text][image15]](https://youtu.be/1LQITwYcbk4)
